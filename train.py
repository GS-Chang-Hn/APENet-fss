...
def main(_run, _config, _log):
    ...
    for i_iter, sample_batched in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        query_outputs_foreground = model(
            support_images, support_fg_mask, support_bg_mask,
            query_images)

        query_loss_true = criterion(query_outputs_foreground, query_labels)
        # loss = torch.tensor(math.exp(sup_loss_true) + math.exp(-sup_loss_false)+math.exp(query_loss_true) + math.exp(-query_loss_false), requires_grad=True)
        # loss = q_f_w * query_loss_true + s_f_w * sup_loss_true
        # loss = query_loss_true
        loss = query_loss_true

        # loss = q_f_w * sup_loss_true - q_b_w * sup_loss_false + s_f_w * query_loss_true - s_b_w * query_loss_false
        loss.backward()
        optimizer.step()
        scheduler.step()
        # Log loss
        ...
        if (i_iter + 1) % _config['print_interval'] == 0:
            ...
            print(
                f'Tranin step {i_iter + 1} -----------------------: average loss: {loss}')
